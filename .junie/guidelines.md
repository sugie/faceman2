...
- LaravelのRefreshDatabaseは使わない。遅いから。
- テストで使うデータはテスト実行時に作成し、テスト終了後は削除しない。テスト実行時に前回のテストデータを削除する。テスト後にデータベースに値が残っていれば、デバッグに役立つから。
- このため、本番環境では決してデータベース更新が行われるテストを実行してはいけない。
